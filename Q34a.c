...
int main() {
    ...
    while (1) {
        ...
        if (fork() == 0) {
            close(server_sock);  // Child process closes its copy of the server socket
            handle_client(client_sock);  // Handle client communication
            exit(0);  // Child process exits after handling client
        }
        close(client_sock);  // Parent process closes its copy of the client socket
    }
...
Received from client: Client 1 will now exit
Client disconnected
Received from client: Client 2 will exit too now
Client disconnected*/